  ...
  for (const { find, replace } of replacements) {
    ...
    while (safety < 30) {
      ...
      if (xml.includes(find)) {
        ...
        count += n;
        console.log('  OK "' + find + '" -> "' + replace + '" (' + n + 'x)');
        continue;
      }
      ...
      if (fullText.includes(find)) {
        ...
        if (startPos >= 0 && endPos >= 0) {
          ...
          count++;
          console.log('  OK (frag) "' + find + '" -> "' + replace + '" (1x)');
          continue;
        }
      }
...
console.log('\n--- 10. Anexa 7 ---');
total += processFileAll('10. Anexa 7 - Acord in cazul schimbarii starii de sanatate.docx', [
  { find: '12.07  .2026', replace: '{DATA_CONTRACT}' },
]);

console.log('\n--- 11. Anexa 8 ---');
total += processFileAll('11. Anexa 8 - Acord de închidere centru.docx', [
  { find: '12.07  .2026', replace: '{DATA_CONTRACT}' },
]);

console.log('\n--- 4. Anexa 1 ---');
total += processFileAll('4. Anexa 1.docx', [
  { find: '12.07  .2026', replace: '{DATA_CONTRACT}' },
]);

console.log('\n--- CONTRACT NOU ---');
total += processFileAll('CONTRACT NOU PRESTĂRI SERVICII.docx', [
  { find: '12.07.2026', replace: '{DATA_CONTRACT}' },
  { find: '12.07 .2026', replace: '{DATA_CONTRACT}' },
...
console.log('\n--- DECLARAȚIE ROF ---');
total += processFileAll('DECLARAȚIE LUARE LA CUNOȘTINȚĂ ROF CLINCENI (1).docx', [
  { find: '12.07.2025', replace: '{DATA_CONTRACT}' },
]);
